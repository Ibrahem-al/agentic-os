...
- No new deps. New `dedupe_scans` table via the base `IF NOT EXISTS` schema, `APPDATA_USER_VERSION` 11 → 12 (additive; migration test `v11 → v12` added). `DEFAULT == TODAY`: omitting `scope` reproduces the prior scan; a fresh install has no `dedupe_scans` row until the first scan.
