...
9. **Stale June-prototype data found in the real userData dir** (`%APPDATA%/agentic-os`): a 5 MB `graph` **file** (blocked our `graph/` dir), an `appdata.db` with `user_version=2` and different table shapes, `backups/`, `keychain.json`, `sample.mcp.json`, `agentic-os/`, `spike-data/`. Two permanent guards were added: the engine throws a clear error when the graph path exists as a non-directory, and `openAppData` refuses a db whose `user_version` is newer than it understands. **Resolved (user-approved):** all nine leftovers were relocated intact into `%APPDATA%/agentic-os/legacy-2026-06-prototype/`; a subsequent `npm run dev` against the default userData booted cleanly (fresh `graph/` at schema v1, fresh `appdata.db` with `user_version=1` and exactly the five §20 tables) and quit gracefully.
