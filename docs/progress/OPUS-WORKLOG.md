...
| 2026-07-06 | 16b | Provider WIRING: 8 non-extraction roles via router? optional dep; boot builds one shared router (subscription inert); settings.save merge + onSettingsChanged invalidate (P1.1) | ultracode Workflow (`wf_cfd254ad-dd9`), Opus agents | index.ts, ipc.ts, skills/{agent,testset,candidate,benchmark,types}.ts, agents/index.ts, retrieval/loop.ts, ingest/codebase.ts, security/scanner.ts, kernel/context.ts, mcp/{server,tools/shared,tools/control}.ts, +tests | **done** — lint+typecheck clean, 732 passed/12 skip offline (latency flake isolated), extraction/* provably unchanged; orchestrator reviewed the boot router build | `phase-16` |
