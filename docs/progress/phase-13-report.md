...
- **The lossless fix (ours, engine-level, third design iteration)**: embeddings are never bound as *bare* `number[]` parameters anymore. Two rejected designs first: inline Cypher literals were lossless but cost **+90 ms per vectorSearch** of statement parsing (the fixture-graph latency pin went 143 → 479 ms), and `Float64Array` binds as a STRUCT (CAST fails). The shipped design (`src/main/storage/ryugraph.ts` — `embeddingParam`/`embeddingParamExpr`/`queryVectorParam`/`sanitizeVectorElements`): **writes** bind a fractional-**sentinel**-prefixed param (element[0] = 0.5 pins the list type to DOUBLE) recovered in Cypher via `list_slice`, with non-zero integral elements nudged by a **float32-invisible** relative epsilon (`v·(1+2⁻²⁶)` rounds back to exactly `v` in the FLOAT[1024] column; zeros survive by bit-identity); **query** vectors (CALL args accept only literals/params — no `list_slice` expression) sanitize the same way, with a zero first element becoming 1e-8 (query-side only, ~1e-16 distance shift, nothing stored changes). `encodeParams` now **refuses** the poison mixes (int-typed list with fractionals, or double-typed list with non-zero integrals) for raw `cypher()` callers instead of silently corrupting. Perf cost: none measurable — the latency pin returned to 143 ms and the 10k benchmark to its baseline.
...
**Numbers (this machine, quiet):** see Verification below — `retrieve()` p50 = 358.1 ms / p95 = 387.8 ms (367.6/399.3 in the earlier under-concurrent-load run; 180-193 ms per query on the ubuntu runner), comfortably under the 500 ms assertion (the phase-03 48-node baseline was 117 ms — the pipeline's graph-size sensitivity is sub-linear thanks to the fused-candidate caps). Deviations: none needed — the pre-identified levers (prepared-statement caching, expansion overlap) stay untouched and recorded as future headroom.
...
1. **Embeddings ride sentinel-prefixed, element-sanitized parameters** (writes recover the vector via `list_slice`; queries sanitize in place) — the lossless, parameter-speed neutralization of the ryugraph 25.9.1 NAPI list-binding fault (element[0]-based type inference + per-element bit-reinterpretation). Inline literals (+90 ms/search) and `Float64Array` (binds as STRUCT) were measured and rejected first — see the RyuGraph story above. `encodeParams` refuses the poison patterns for raw callers. Filing upstream is the follow-up; the §20 pin stands (25.9.1 is the latest release).
