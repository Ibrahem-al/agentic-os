/**
 * Automations panel (phase 31, promoted to a top-level tab in phase 33) — the
 * dashboard authoring surface for user-defined scheduled tasks & watchers
 * (spec agent #5). Lists every automation with an on/off switch, run-now, edit
 * and delete; "New automation" opens the editor; broken rule files surface as
 * verbatim error rows with a delete-to-clean action. Every mutation is applied
 * LIVE by the backend (no restart) and is undoable from the toast or History.
 *
...
export default function AutomationsPanel(): React.JSX.Element {
  ...
  const runNow = async (rule: RuleDetailDto): Promise<void> => {
    ...
    try {
      ...
      toast.notify(
        'ok',
        `Queued '${rule.id}' — see Background work.${rule.action.kind === 'code' ? ' Its first write or network call may wait in Approvals.' : ''}`
      )
    } catch (err) {
      toast.notify('err', msg(err))
    } finally {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <PanelHeader
        title="Automations"
        subtitle="Run a job on a schedule, or when a file or web page changes"
        icon={<Icon name="automations" size={18} />}
        actions={
      ...
      <div className="min-w-0 flex-1 overflow-y-auto px-5 py-4" data-testid="automations-panel">
        {rules.error !== null ? (
      ...
      {editorTarget !== null && (
        <Modal title={editorInitial !== null ? 'Edit automation' : 'New automation'} wide onClose={() => setEditorTarget(null)}>
          <AutomationEditor
