...
**Editing memory.** The memory browser is fully editable from the dashboard — add a preference or note, edit or connect existing memories, remove one (deleting a document takes its saved chunks with it), or run *Find duplicates* to review and merge near-identical entries — group by group, or all of them in one step (one undo restores the whole batch). You can also hand the sweep to the AI: *Let AI clean up* runs a background scan that has the local model judge the near-matches and files its merge proposals in **Approvals** for your review (Claude can trigger the same sweep over MCP with `run_graph_cleanup`) — nothing merges without your approval. Every change is written through the same audited lane the background agents use, so **anything you do is reversible with one click from the History panel** (the toast even offers an inline Undo). Adding or renaming a *searchable* memory — a preference, note, skill, or project — re-embeds it, so those actions need the local AI helper (Ollama) running; tags and plain links work fully offline. Editing here is a human-only, dashboard-only path: Claude over MCP still writes only by proposing corrections you approve.
